...
interface CustodyDetailsModalProps {
  ...
  assetsInLog: Asset[];
  onClose: () => void;
  onDischarge: (logId: number) => Promise<void>;
  onUploadSignedTerm: (logId: number, fileUrl: string) => Promise<void>;
}
...
const CustodyDetailsModal: React.FC<CustodyDetailsModalProps> = ({ log, user, assetsInLog, onClose, onDischarge, onUploadSignedTerm }) => {
  ...
  const handlePrint = () => {
    ...
    doc.setFontSize(14);
    doc.text(`Nº: ${log.cautela_number}`, doc.internal.pageSize.width / 2, 28, { align: 'center' });
    ...
    const tableRows: (string|undefined)[][] = assetsInLog.map(asset => [asset.qr_code, asset.name, asset.serial_number, asset.patrimony_id]);
    ...
    doc.save(`termo_cautela_${log.cautela_number.replace(/\//g, '-')}.pdf`);
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        await onUploadSignedTerm(log.id, fakeUrl);
      } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={onClose}>
      <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Detalhes da Cautela - {log.cautela_number}</h2>
        ...
        <div className="flex-grow overflow-y-auto mb-6 pr-4 -mr-4">
          <div className="grid grid-cols-2 gap-4 text-sm mb-4">
            <div><strong>Data da Cautela:</strong> {new Date(log.checkout_date).toLocaleDateString('pt-BR')}</div>
            <div><strong>Status:</strong> {log.checkin_date ? <span className="font-bold text-gray-600">Concluída em {new Date(log.checkin_date).toLocaleDateString('pt-BR')}</span> : <span className="font-bold text-green-600">Ativa</span>}</div>
          </div>
                ...
                <th scope="col" className="px-6 py-3">Nº de Série</th>
                          ...
                          {assetsInLog.map(asset => (
                            <tr key={asset.id} className="bg-white border-b hover:bg-gray-50">
                              <td className="px-6 py-4 font-mono">{asset.qr_code}</td>
                              <td className="px-6 py-4 font-medium text-gray-900">{asset.name}</td>
                              <td className="px-6 py-4">{asset.category}</td>
                              <td className="px-6 py-4">{asset.patrimony_id || 'N/A'}</td>
